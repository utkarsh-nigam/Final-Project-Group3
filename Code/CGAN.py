...
def plt_img(gan,id):
    ...
    for i in range(r):
        for j in range(c):
            ...
            axs[i,j].set_title("Cat type: %d" % noise_label[cnt])
            axs[i,j].axis('off')
            cnt += 1
    plt.title("step = "+str(id))
...
for learning_step in range(LEARNING_STEPS):
    ...
    if (learning_step + 1) % 10 == 0:
        plt_img(gan,learning_step+1)

    if (learning_step + 1) % 10 == 0:
        gan.gen.save('himalayan_%d.h5' % ((learning_step + 1) * 100))
